	...
	switch(m_ActiveWeapon)
	{
		case WEAPON_HAMMER:
		{
			...
			for(int i = 0; i < Num; ++i)
			{
				...
				if((pTarget == this) || (!Config()->m_SvHammerIgnoreTiles && GameServer()->Collision()->IntersectLine(ProjStartPos, pTarget->m_Pos, NULL, NULL)))
					continue;
...
void CCharacter::Snap(int SnappingClient)
{
	...
	{
		SetEmote(GetPlayer()->m_DefaultEmote, -1);
	}
		...
		(!Config()->m_SvStrictSpectateMode && m_pPlayer->GetCID() == GameServer()->m_apPlayers[SnappingClient]->GetSpectatorID()))
	{
		...
		else if(m_aWeapons[m_ActiveWeapon].m_Ammo >= -1)
